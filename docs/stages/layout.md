...
6. Write subspecs to `tests/features/`; compile `delivery-order.yaml` from subspec `Requires:` and `Provides:` declarations to `.haileris/features/{feature_id}/delivery-order.yaml`.
...
- ANLZ-003, ANLZ-004, and ANLZ-006 run at Layout.Verify, after subspecs and `@traces` tags exist
